...
int main() {
  Graph graph;
  const char *vertices_to_add[NUM_VERTICES]= {
    "koala", "platypus", "snake", "salamander", "gecko", "frog", "dog",
    ...
    for (j= 0; j < NUM_VERTICES; j++)
      if (edges[i][j] != 0)
        assert(get_edge_cost(graph, vertices_to_add[i], vertices_to_add[j]) ==
               edges[i][j]);
      else assert(get_edge_cost(graph, vertices_to_add[i],
                                vertices_to_add[j]) == -1);
  ...
  for (i= 0; i < NUM_VERTICES; i++)
    for (j= 0; j < NUM_VERTICES; j++)
      if (edges[i][j] != 0)
        change_edge_cost(&graph, vertices_to_add[i], vertices_to_add[j],
                         edges[i][j] + 1);
